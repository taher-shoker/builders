...
export class KpisPerformanceComponent implements OnInit {
  ...
  kpisObjectiveTwo: Dropdown[] = [
    { id: 0, name: 'All', kpiProperty: 'objectiveTwo' },
  ...
  kpisObjectiveThree: Dropdown[] = [
    { id: 0, name: 'All', kpiProperty: 'objectiveThree' },
  ...
  kpisUnits: Dropdown[] = [
    { id: 0, name: 'All', kpiProperty: 'unit' },
    { id: 1, name: 'Weeks', kpiProperty: 'unit' },
    { id: 1, name: 'Days', kpiProperty: 'unit' },
    { id: 1, name: '%', kpiProperty: 'unit' },
    { id: 1, name: '#', kpiProperty: 'unit' },
  ];
  ...
  fetchKpiDetails(kpiItem: KpiData) {
    ...
    this.destroyChartView();

    console.log('kpiItem in the PERFORMANCE :', kpiItem);
  ...
  fetchFilters() {
    this.dataService.getFilters().subscribe((res: Filters) => {
      console.log('Da res', res);

      this.populateFilterArrays(
        res.kpi_dimension,
        'kpisDimensions',
        this.kpisDimensions[0].kpiProperty
      );
      this.populateFilterArrays(
        res.objective_level_2,
        'kpisObjectiveTwo',
        this.kpisObjectiveTwo[0].kpiProperty
      );
      this.populateFilterArrays(
        res.objective_level_3,
        'kpisObjectiveThree',
        this.kpisObjectiveThree[0].kpiProperty
      );
      this.populateFilterArrays(
        res.unit,
        'kpisUnits',
        this.kpisUnits[0].kpiProperty
      );
  ...
  populateFilterArrays(
    ...
    const firstEl = this[arrayName][0];

    this[arrayName] = [];

    for (let i = 0; i < filters.length; i++) {
      this[arrayName].push({ id: i, name: filters[i], kpiProperty });
    }

    this[arrayName].unshift(firstEl);
  }
  ...
  destroyChartView() {
    if (this.kpiDetails) {
      this.kpiDetails.length = 0;
    }

    if (this.lineChartData && this.lineChartTarget) {
      this.lineChartData.length = 0;
      this.lineChartTarget.length = 0;
    }
  }
  ...
  filterWith({ name, kpiProperty }: { name: string; kpiProperty: string }) {
    ...
    if (idx >= 0) {
      if (name === 'All') {
        this.currentChosenFilters.splice(idx, 1);
      } else {
