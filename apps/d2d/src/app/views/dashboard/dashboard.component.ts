...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterProductivityGroup
      .get('filterValue')
      ?.setValue(this.dateFilterOption[1]);
  ...
  getDashboardInsights() {
    ...
    this.dashboardService.getStatusChartData(
      this.startDate.toLocaleDateString('sv'),
      this.endDate.toLocaleDateString('sv')
    );
  }
  ...
  getProductivityChartData(fromDate: string, toDate: string) {
    this.dashboardService
      .getProductivityChartData(fromDate, toDate)
      .subscribe((data) => {
  ...
  getStatusChartData(fromDate?: string, toDate?: string) {
    this.dashboardService
      .getStatusChartData(fromDate, toDate)
      .subscribe((data) => {
        const newData: { name: string; value: number }[] = [];

        data.data.forEach(
          (chart: { caseStatus: string; caseCount: number }) => {
            newData.push({
  ...
  getTrendChartData(
    ...
    this.dashboardService
      .getWeeklyTrendChartData(user, team, fromDate, toDate)
      .subscribe((data) => {
        const newData: {
  ...
  getChartTypesData(fromDate?: string, toDate?: string) {
    this.dashboardService
      .getChartTypesData(fromDate, toDate)
      .subscribe((data) => {
        ...
        this.casesTypes = {
          ...
          totalCases:
            this.dashboardUserCases && this.dashboardUserCases.allCaseCount
              ? this.dashboardUserCases.allCaseCount
              : 0,
          chartData: newData,
        };
      });
  ...
  filterLineChartWithDate(filterObj: YearRangeObj) {
    if (this.filterValueForm.get('filterValue')?.value.email) {
      this.getTrendChartData(
        filterObj.fromDate,
        filterObj.toDate,
        this.filterValueForm.get('filterValue')?.value.email
      );
    } else {
      this.getTrendChartData(
        filterObj.fromDate,
        filterObj.toDate,
        undefined,
        this.filterValueForm.get('filterValue')?.value.name
      );
    }
  }
  ...
  filterProgressCircle(newDate: DateRange) {
    this.getChartTypesData(
      newDate.fromDate.toLocaleDateString('sv'),
      newDate.toDate.toLocaleDateString('sv')
    );
  }
  ...
  filterProductivityUser(newDate: DateRange) {
    //dateObj : {name: string, id: number}
