...
export class AddPsrProjectFormComponent implements OnInit {
  ...
  private getProjectValuesById(gd: string, id: number) {
    this.psrService.getProjectById(gd, id).subscribe({
      next: (res: PSRProjectDetailsModel) => {
        ...
        this.programsList.controls[0]
          .get('startDate')
          ?.setValue(new Date(res.startDate));
        this.programsList.controls[0]
          .get('endDate')
          ?.setValue(new Date(res.endDate));
        this.programsList.controls[0].get('POAmount')?.setValue(res.poAmount);
        this.programsList.controls[0]
          .get('actualSpending')
          ?.setValue(res.actualSpending);
  ...
  getErrorMessage(program: AbstractControl, controlName: string): string {
    ...
    if (
      control?.touched &&
      (control?.hasError('required') || control?.hasError('noSpaces'))
    ) {
      errorMag = 'This field is required.';
    } else if (control?.hasError('max')) {
      errorMag = `This field has max value ${
        control.errors ? control.errors['max'].max : 100
      }`;
    }
    return errorMag;
  }
  ...
  changeEndDate(endDate: Date, index: number) {
    ...
    if (
      this.programsList.controls[index].get('startDate')?.hasError('required')
    ) {
  ...
  editProgram() {
    ...
    if (this.inPSRForm) {
      ...
      const addedProjects: AddProjectModel = {
        ...
        indicator:
          updatedObj.indicator && updatedObj.indicator.value
            ? updatedObj.indicator.value
            : updatedObj.indicator,
