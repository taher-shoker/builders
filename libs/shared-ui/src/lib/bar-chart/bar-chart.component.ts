...
export class BarChartComponent implements OnInit , AfterViewInit , OnDestroy, OnChanges{
  ...
  initBarChart() {
    ...
    series.columns.template.adapters.add("fill", function (fill, target:any) {
      return target.dataItem ? am5.color(target.dataItem.dataContext["color"]) : am5.color("#4f008c");
    });
    ...
    if(this.isFinancialStatus)
    {
      ...
      const legend = chart.children.push(
        am5.Legend.new(this.root, {
          ...
          x: am5.percent(50),
          layout : window.innerWidth < 1300 ? this.root.gridLayout : this.root.horizontalLayout,
        })
      );
